---
title: "Statistical Details: Within- and Between-Group Correlation Methods"
...
## Why two correlations instead of one

When observations are nested within groups (repeated measurements per person,
students within classrooms, employees within teams, ...), a variable can vary
both *within* a group (e.g., a person's happiness fluctuating from day to day) and
*between* groups (e.g., some people being generally happier than others on
average). A single correlation computed while ignoring the grouping structure
mixes these two sources of variation together, and can differ in size --- or
even in sign --- from either the within-group or the between-group
relationship. `within_between_correlations()` separates the two, using one of
...
| | `method = "decomposition"` | `method = "sem"` |
|---|---|---|
| **Speed** | Fast; closed-form | Slower; iterative MLR estimation |
| **Balanced group sizes** | Works well | Works well |
| **Very unequal group sizes** | `weight = TRUE` corrects the *point estimate*; significance test always uses unweighted group means (see above) | Handled natively by ML for both point estimate and significance test |
| **Missing data** | Pairwise deletion per variable pair | Listwise deletion across all modeled variables |
| **Small number of groups** | Significance tests are exact | Asymptotic MLR standard errors; can be unreliable with few groups |
| **Interpretability** | Simple, transparent formulas | Estimates come from a fitted latent-variable model |
...
estimate --- to fully account for that imbalance.
