...
(`r sprintf("%.4f", fixef(fit_rewb)["screen_time_within"])`) is positive and highly
...
`r sprintf("%.4f", fixef(fit_rewb)["screen_time_within"])` points higher. For a person
watching 60 minutes more than usual, the expected gain would be
`r sprintf("%.2f", 60 * fixef(fit_rewb)["screen_time_within"])` wellbeing points.

The between-person coefficient (`r sprintf("%.4f", fixef(fit_rewb)["screen_time_between"])`)
is negative and significant. Illustrating interpretation: people who watch one
minute more per day on average show
`r sprintf("%.4f", abs(fixef(fit_rewb)["screen_time_between"]))` lower wellbeing. For
someone who watches 60 minutes more per day on average than another person, the
expected wellbeing gap would be
`r sprintf("%.2f", abs(60 * fixef(fit_rewb)["screen_time_between"]))` points.
...
(`r sprintf("%.4f", fixef(fit_rewb_conf)["screen_time_within"])`): `self_control` is a
...
`r sprintf("%.4f", fixef(fit_rewb)["screen_time_between"])` (significant) in the
unadjusted model to
`r sprintf("%.4f", fixef(fit_rewb_conf)["screen_time_between"])`
(*p* = `r sub("^0", "", sprintf("%.2f", coef(summary(fit_rewb_conf))["screen_time_between", "Pr(>|t|)"]))`,
