# Shared setup for test-bayes_mldesc.R and test-bayes_within_between_correlations.R.
#
# Both files fit real brms models, which is by far the slowest part of the
# test suite. To keep that bearable:
#
# 1. Both files use the *same* cache folder and the *same* fixture data
#    objects (defined once, here). bayes_mldesc() calls
#    bayes_within_between_correlations() internally with the data/vars/folder
#    it was given, and brms::brm(file = ...) caches purely on a hash of that
#    data plus the sampling settings (see `.brms_iter()`/`.brms_chains()` in
#    R/utils.R). So whichever of the two test files runs first fits the
#    within/between correlation models, and the other reuses them from disk
#    instead of refitting -- as long as the fixture data is byte-identical.
